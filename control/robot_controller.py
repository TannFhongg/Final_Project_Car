...
Updated with PID-based Auto Mode for lane following
"""
...
class AutoModeController:
    ...
    Uses camera for lane detection and PID for smooth steering
    """
    ...
    def __init__(self, robot_controller: RobotController):
        ...
        # Camera
        self.camera = None
        self.camera_device = robot_controller.config.get('sensors', {}).get('camera', {}).get('device', 0)
        ...
        self.pid = PIDController(
            ...
            output_min=-150,  # Max steering correction
            output_max=150,
        ...
        logger.info("Auto Mode Controller initialized with PID")
    ...
    def start(self):
        ...
        if not self.running:
            ...
            self.thread.start()
            logger.info("Autonomous mode started (PID lane following)")
            return True
        return False
    ...
    def _init_camera(self) -> bool:
        ...
        try:
            ...
            # Set resolution
            camera_config = self.robot.config.get('sensors', {}).get('camera', {})
            width, height = camera_config.get('resolution', [640, 480])
            self.camera.set(cv2.CAP_PROP_FRAME_WIDTH, width)
            self.camera.set(cv2.CAP_PROP_FRAME_HEIGHT, height)
            self.camera.set(cv2.CAP_PROP_FPS, camera_config.get('framerate', 30))
    ...
    def _auto_loop(self):
        ...
        """
        logger.info("Auto loop started (PID mode)")
    ...
    def get_debug_frame(self):
        """Get latest debug frame for visualization"""
